...
```
🔍 三路检索      向量(BGE) + BM25(jieba) + 图谱([[wikilinks]] + 共现词)
                 → RRF 融合(权重自适应) → Cross-Encoder 精排
                 关键词自动增强 (正则+TF-IDF+TextRank, LLM兜底)
                 时间衰减 (30天半衰期) + Reranker 评分缓存

⚡ 查询加速      常驻 HTTP 服务 (kb_server), 模型只加载一次
                 --fast 模式跳过 CrossEncoder, 纯 RRF 融合, 50ms 极速
                 Server 自动检测, 不可用时回退本地加载
...
│ query / kb_server (常驻服务) / incremental_ingest│
│ lifecycle / health                              │
├────────────────────────────────────────────────┤
│ L3: 系统定时任务                                │
...
Claude Code / CLI
     │
     ├─ python query.py "..." --fast    (推荐日常)
     │     │
     │     ├─ kb_server 在线? ──→ HTTP /query ──→ 50ms ⚡
     │     └─ kb_server 离线? ──→ 本地加载 (~30s)
     │
     └─ python kb_server.ps1 start     (会话开始时执行一次)
           └─ 模型常驻内存, Reranker 评分 LRU 缓存
```

| 模式 | 首次查询 | 缓存命中 | 适用场景 |
|------|:--:|:--:|------|
| `--fast` | 52ms | 50ms | 日常检索 (RRF 融合) |
| 标准 (无 --fast) | ~6s | 50ms | 需要最高精度 (CrossEncoder) |
| 本地回退 | ~36s | ~16s | Server 不可用时 |
...
# 启动常驻查询服务（只需一次，模型常驻内存）
python kb_server.ps1 start           # Windows
# python kb_server.py &              # Linux/Mac

python -m vaultrag.ingest            # 首次建索引
python -m vaultrag.query "搜索" --fast  # 检索 (50ms 极速)
...
query
  ...
  │   └─ [[wikilinks]] + 共现词 → 图谱扩展
  ├─ RRF 融合 (k=60, 权重自适应)
  ├─ [可选] Cross-Encoder Reranker 精排 + LRU 缓存
  │   --fast 跳过此步 → 50ms 极速
...
├── vaultrag/
│   ├── config.py              统一配置
│   ├── query.py                三路检索 + Server检测 + --fast
│   ├── kb_server.py            常驻HTTP查询服务 (模型常驻, 评分缓存)
│   ├── kb_server.ps1           服务管理脚本 (start/stop/status)
│   ├── ingest.py               全量入库
